...
-- This block mirrors 20260723_state_recovery_ledger_preconditions.sql.
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_qd_order_state_events_versions') THEN
        ALTER TABLE qd_order_state_events ADD CONSTRAINT chk_qd_order_state_events_versions
            CHECK ((expected_version IS NULL AND resulting_version IS NULL) OR (expected_version >= 0 AND resulting_version = expected_version + 1)) NOT VALID;
    END IF;
END $$;
CREATE UNIQUE INDEX IF NOT EXISTS uq_qd_order_state_events_idempotency
    ON qd_order_state_events(economic_order_id, idempotency_key) WHERE idempotency_key IS NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS uq_qd_order_state_events_fingerprint
    ON qd_order_state_events(economic_order_id, event_fingerprint) WHERE event_fingerprint IS NOT NULL;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_qd_submission_attempts_version_sequence') THEN
        ALTER TABLE qd_submission_attempts ADD CONSTRAINT chk_qd_submission_attempts_version_sequence
            CHECK (version >= 0 AND last_event_seq >= 0) NOT VALID;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'uq_qd_submission_attempts_id_economic_order') THEN
        ALTER TABLE qd_submission_attempts ADD CONSTRAINT uq_qd_submission_attempts_id_economic_order
            UNIQUE (id, economic_order_id);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_qd_submission_attempts_capability_snapshot') THEN
        ALTER TABLE qd_submission_attempts ADD CONSTRAINT fk_qd_submission_attempts_capability_snapshot
            FOREIGN KEY (venue_capability_snapshot_id) REFERENCES qd_venue_capability_snapshots(id) ON DELETE RESTRICT NOT VALID;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_qd_submission_attempts_recovery_policy_snapshot') THEN
        ALTER TABLE qd_submission_attempts ADD CONSTRAINT fk_qd_submission_attempts_recovery_policy_snapshot
            FOREIGN KEY (recovery_policy_snapshot_id) REFERENCES qd_submission_recovery_policy_snapshots(id) ON DELETE RESTRICT NOT VALID;
    END IF;
END $$;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_qd_exchange_fill_events_quote_quantity_origin') THEN
        ALTER TABLE qd_exchange_fill_events ADD CONSTRAINT chk_qd_exchange_fill_events_quote_quantity_origin
            CHECK (quote_quantity_origin IS NULL OR quote_quantity_origin IN ('VENUE','DERIVED')) NOT VALID;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_qd_exchange_fill_events_fee_summary_state') THEN
        ALTER TABLE qd_exchange_fill_events ADD CONSTRAINT chk_qd_exchange_fill_events_fee_summary_state
