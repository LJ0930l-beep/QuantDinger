...
('strategy_v2_double_ma', 'script', 'Dual Moving Average', 'A parameterized BTC perpetual dual-moving-average strategy with an explicit 50x-100x Gate leverage contract.', $double$"""
Dual Moving Average
...
('strategy_v2_macd_kdj', 'script', 'MACD and KDJ Confirmation', 'A BTC perpetual strategy combining MACD momentum, stochastic KDJ confirmation, and an explicit 50x-100x Gate leverage contract.', $macdkdj$"""
...
def handle_data(context, data):
    rsi_period = int(context.params.get("rsi_period", 14))
    oversold = float(context.params.get("oversold", 30))
    overbought = float(context.params.get("overbought", 70))
    ...
    if value != value or previous != previous:
        return
    position = get_position(g.symbol)
    amount = float(position.amount or 0.0)
    if previous <= oversold < value and amount <= 0:
        order_target_percent(g.symbol, target_pct, reason="rsi_5m_entry")
    elif previous >= overbought > value and amount > 0:
        order_target_percent(g.symbol, 0.0, reason="rsi_5m_exit")
$rsiscalper$, '{"params":[{"name":"rsi_period","type":"integer","default":14,"min":2,"max":50,"step":1,"labelKey":"strategyV2.params.rsiPeriod"},{"name":"oversold","type":"number","default":30,"min":5,"max":50,"step":1,"labelKey":"strategyV2.params.oversold"},{"name":"overbought","type":"number","default":70,"min":50,"max":95,"step":1,"labelKey":"strategyV2.params.overbought.label"},{"name":"target_pct","type":"percent","default":0.25,"min":0.05,"max":1,"step":0.05,"labelKey":"strategyV2.params.targetPosition"}]}'::jsonb, '["strategy-v2","cta","ta-lib","rsi","crypto","swap","short-term","5m"]'::jsonb, 'thunderbolt', 'cyan', 75, TRUE, '{"source":"system_seed","version":10,"apiVersion":2,"timeframe":"5m","strategy_leverage_min":50,"strategy_leverage_cap":100}'::jsonb, NOW()),

('strategy_v2_breakout_15m', 'script', 'Donchian 15m Breakout', 'A 15-minute ETH perpetual breakout template with an explicit 50x-100x Gate leverage contract.', $breakoutfifteen$"""
Donchian 15m Breakout
Short-horizon ETH perpetual breakout with a bounded Donchian channel and ATR stop guard.
"""

# @param lookback int 20 range=5:100:1
# @param target_pct float 0.25 range=0.05:1:0.05
# @param atr_period int 14 range=2:50:1
# @param atr_multiplier float 2.5 range=0.5:6:0.25

def initialize(context):
    g.symbol = "Crypto:ETH/USDT@swap"
    context.set_universe([g.symbol])
    context.set_benchmark("Crypto:ETH/USDT@spot")
...
def handle_data(context, data):
    ...
    target_pct = float(context.params.get("target_pct", 0.25))
    atr_period = int(context.params.get("atr_period", 14))
    atr_multiplier = float(context.params.get("atr_multiplier", 2.5))
    bars = get_history(max(lookback, atr_period) + 3, "15m", ["high", "low", "close"], g.symbol)
    atr = indicator("ATR", g.symbol, timeperiod=atr_period)
    if len(bars) < max(lookback, atr_period) + 2 or len(atr) < 2:
        return
    close = float(bars["close"].iloc[-1])
    previous_close = float(bars["close"].iloc[-2])
    upper = float(bars["high"].iloc[:-1].tail(lookback).max())
    lower = float(bars["low"].iloc[:-1].tail(lookback).min())
    atr_value = float(atr.iloc[-1])
    if any(value != value for value in (close, previous_close, upper, lower, atr_value)):
        return
    position = get_position(g.symbol)
    amount = float(position.amount or 0.0)
    if close > upper and amount <= 0:
        order_target_percent(g.symbol, target_pct, reason="breakout_15m_entry")
    elif amount > 0 and (close < lower or close < previous_close - atr_multiplier * atr_value):
        order_target_percent(g.symbol, 0.0, reason="breakout_15m_exit")
$breakoutfifteen$, '{"params":[{"name":"lookback","type":"integer","default":20,"min":5,"max":100,"step":1,"labelKey":"strategyV2.params.lookback"},{"name":"target_pct","type":"percent","default":0.25,"min":0.05,"max":1,"step":0.05,"labelKey":"strategyV2.params.targetPosition"},{"name":"atr_period","type":"integer","default":14,"min":2,"max":50,"step":1,"labelKey":"strategyV2.params.atrPeriod"},{"name":"atr_multiplier","type":"number","default":2.5,"min":0.5,"max":6,"step":0.25,"labelKey":"strategyV2.params.atrMultiplier"}]}'::jsonb, '["strategy-v2","cta","ta-lib","breakout","atr","crypto","swap","short-term","15m"]'::jsonb, 'thunderbolt', 'purple', 76, TRUE, '{"source":"system_seed","version":10,"apiVersion":2,"timeframe":"15m","strategy_leverage_min":50,"strategy_leverage_cap":100}'::jsonb, NOW()),
