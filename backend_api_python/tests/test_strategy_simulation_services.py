...
from app.domain.gate_market_payload_contracts import AssetMarketType as GatePayloadAssetMarketType, normalize_gate_candles  # noqa: E402
...
class StrategySimulationServiceTests(unittest.TestCase):
    def test_strategy_factory_enforces_dataset_scope_without_guessing(self):
        factory = StrategyFactory()
        signal = factory.generate_signal(
            _strategy(StrategyFamily.SMC), _bars(), signal_id="scoped-signal",
            data_snapshot_id="snapshot-1", timeframe="1m", market_type="spot",
        )
        self.assertEqual(signal.signal_id, "scoped-signal")
        with self.assertRaises(StrategyFactoryError):
            factory.generate_signal(
                _strategy(StrategyFamily.SMC), _bars(), signal_id="wrong-timeframe",
                data_snapshot_id="snapshot-1", timeframe="5m", market_type="spot",
            )
        with self.assertRaises(StrategyFactoryError):
            factory.generate_signal(
                _strategy(StrategyFamily.SMC), _bars(), signal_id="missing-market",
                data_snapshot_id="snapshot-1", timeframe="1m",
            )
    ...
    def test_gate_dataset_enters_the_same_strategy_risk_paper_pipeline(self):
        ...
        candles = normalize_gate_candles(
            raw, market_type=GatePayloadAssetMarketType.SPOT, instrument_id="BTC_USDT", interval="1m",
    ...
    def test_production_readiness_is_evidence_only_and_never_enables_live(self):
        ...
        execution_missing = ProductionReadinessEvidence(True, True, True, True, True, True, True, True, False, False)
        self.assertEqual(derive_production_readiness(execution_missing), ProductionReadinessStatus.BLOCKED)
        execution_verified = ProductionReadinessEvidence(True, True, True, True, True, True, True, True, False, True)
        self.assertEqual(derive_production_readiness(execution_verified), ProductionReadinessStatus.PRODUCTION_READY)
        recovery_missing = ProductionReadinessEvidence(
            True, True, True, True, True, True, True, True,
            live_enabled=False, testnet_execution_passed=True, paper_recovery_passed=False,
        )
        self.assertEqual(derive_production_readiness(recovery_missing), ProductionReadinessStatus.BLOCKED)
