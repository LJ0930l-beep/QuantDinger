...
class StrategySignalTests(unittest.TestCase):
    ...
    def test_all_builtin_families_emit_deterministic_typed_signals(self):
        ...
        for family in (S.StrategyFamily.EMA_ADX_TREND, S.StrategyFamily.DONCHIAN_ATR, S.StrategyFamily.BOLLINGER_RSI, S.StrategyFamily.DUAL_THRUST):
    ...
    def test_dual_thrust_breakout_is_typed_and_uses_threshold_entry(self):
        values = bars(((100, 103, 98, 101), (101, 105, 99, 104), (104, 106, 100, 102), (102, 105, 101, 103), (104, 110, 103, 108)))
        definition = S.StrategyDefinition("dual", "v1", S.StrategyFamily.DUAL_THRUST, "schema", "dataset-1", (S.StrategyParameterFact("lookback", "4"), S.StrategyParameterFact("upper_factor", "0.5"), S.StrategyParameterFact("lower_factor", "0.5")))
        signal = M.build_strategy_signal(definition, values, signal_id="sig", data_snapshot_id="d")
        self.assertEqual(signal.direction, S.SignalDirection.BUY)
        self.assertEqual(signal.entry_price, Decimal("107"))
        self.assertEqual(signal.strategy.family, S.StrategyFamily.DUAL_THRUST)
