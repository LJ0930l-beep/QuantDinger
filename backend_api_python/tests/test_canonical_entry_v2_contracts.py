...
def request(action=OrderAction.OPEN, x=None):
    actor=EntryActorContext(Actor.PROTECTION,'protect',EntrySource.PROTECTION) if action is OrderAction.PROTECTION else EntryActorContext(Actor.HUMAN,'human',EntrySource.REST)
    ...
    return CanonicalEntryRequestV2(1,2,'account','BTCUSDT','swap',action,x or intent(close=effect is RiskEffect.REDUCE_RISK),actor,effect)
class V2Tests(unittest.TestCase):
 ...
 def test_close_all_cancel_and_subjects(self):
  ...
  cancel=request(OrderAction.CANCEL,intent(cancel=True)); DurableEntryGraphV2('a',cancel,CancelTargetSubject(CancelTargetKind.ECONOMIC_ORDER_ID,'order-1'))
  with self.assertRaises(CanonicalEntryV2Error): DurableEntryGraphV2('a',cancel,EconomicOrderSubject('x'))
