...
from .canary_release_contracts import CanaryDecision, CanaryPromotionResult
...
def derive_deployment_readiness_with_canary(
    profile: DeploymentReleaseProfile,
    production_status: ProductionReadinessStatus,
    canary_result: CanaryPromotionResult,
) -> DeploymentReadinessStatus:
    """Apply the canary gate before allowing a canary/production candidate.

    The legacy two-argument function remains compatible for existing read-only
    artifacts.  New promotion tooling should use this stricter form so a
    production profile cannot be considered ready without a typed canary
    decision and rollback evidence.
    """

    if not isinstance(canary_result, CanaryPromotionResult):
        raise DeploymentReadinessError("typed canary promotion result is required")
    status = derive_deployment_readiness(profile, production_status)
    if profile.environment in (DeploymentEnvironment.CANARY, DeploymentEnvironment.PRODUCTION):
        if canary_result.decision is not CanaryDecision.PROMOTION_CANDIDATE:
            return DeploymentReadinessStatus.BLOCKED
    return status
