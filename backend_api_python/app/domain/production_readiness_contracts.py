...
class ProductionReadinessEvidence:
    ...
    # ``None`` preserves legacy evidence artifacts that predate the execution
    # rehearsal flag.  New Gate TestNet release evidence must set this to
    # True; an explicit False is fail-closed.
    testnet_execution_passed: bool | None = None
    paper_recovery_passed: bool | None = None
    ...
    def __post_init__(self) -> None:
        ...
        if self.testnet_execution_passed is not None and not isinstance(self.testnet_execution_passed, bool):
            raise ProductionReadinessError("testnet_execution_passed must be boolean when supplied")
        if self.paper_recovery_passed is not None and not isinstance(self.paper_recovery_passed, bool):
            raise ProductionReadinessError("paper_recovery_passed must be boolean when supplied")
        ...
        material = {
            ...
            "testnet_execution_passed": self.testnet_execution_passed,
            "paper_recovery_passed": self.paper_recovery_passed,
            "live_enabled": False,
        }
...
def derive_production_readiness(evidence: ProductionReadinessEvidence) -> ProductionReadinessStatus:
    ...
    if evidence.testnet_execution_passed is False:
        return ProductionReadinessStatus.BLOCKED
    if evidence.paper_recovery_passed is False:
        return ProductionReadinessStatus.BLOCKED
