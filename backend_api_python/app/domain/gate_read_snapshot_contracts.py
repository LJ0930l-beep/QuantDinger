...
class GateReadSnapshot:
    ...
    def __post_init__(self) -> None:
        ...
        for fact in (*self.balances, *self.instruments, *self.positions, *self.market_facts):
        ...
        expected = build_gate_read_snapshot_fingerprint(self.auth, self.balances, self.instruments, self.positions, self.market_facts, observed)
...
def build_gate_read_snapshot_fingerprint(
    ...
    for group in (balances, instruments, positions, market_facts):
...
def build_gate_read_snapshot(
    ...
    *,
    ...
    try:
        fingerprint = build_gate_read_snapshot_fingerprint(auth, balances, instruments, positions, market_facts, observed_at)
        return GateReadSnapshot(auth, balances, instruments, positions, market_facts, observed_at, fingerprint)
