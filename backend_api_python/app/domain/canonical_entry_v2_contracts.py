...
from app.domain.canonical_entry_contracts import EntryActorContext, ExecutionKind, OrderSide, PositionSide, CanonicalEntryRequest
...
class CanonicalEntryRequestV2:
    tenant_id:int; credential_id:int; account_scope:str; instrument_id:str; market_type:str; action:OrderAction; economic_intent:CanonicalEconomicIntentV2; actor:EntryActorContext; risk_effect:RiskEffect
    economic_fingerprint:str=field(init=False)
    def __post_init__(self):
        if self.tenant_id<=0 or self.credential_id<=0 or not isinstance(self.action,OrderAction) or not isinstance(self.actor,EntryActorContext): raise CanonicalEntryV2Error("invalid V2 scope")
        ...
        if self.action is OrderAction.PROTECTION and self.actor.actor_type is not Actor.PROTECTION: raise CanonicalEntryV2Error("protection actor required")
...
def convert_v1_non_stop(request: CanonicalEntryRequest) -> CanonicalEntryRequestV2:
    ...
    if intent.execution_kind in (ExecutionKind.STOP_MARKET,ExecutionKind.STOP_LIMIT): raise CanonicalEntryV2Error("V1 STOP cannot convert without trigger direction and price type")
    return CanonicalEntryRequestV2(request.tenant_id,request.credential_id,request.account_scope,request.instrument_id,request.market_type,request.action,CanonicalEconomicIntentV2(intent.side,intent.quantity,QuantitySemantics.ABSOLUTE if intent.quantity else None,intent.execution_kind,intent.limit_price,None,None,None,intent.reduce_only,intent.position_side,None,intent.cancel_target_id,intent.target_position_id,intent.close_quantity,intent.close_all),request.actor,request.risk_effect)
