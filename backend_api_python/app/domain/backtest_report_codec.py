...
def serialize_backtest_report(report: BacktestReportSnapshot) -> dict[str, Any]:
    ...
    return {
        ...
        "run": {
            ...
            "cost_policy_fingerprint": run.cost_policy_fingerprint,
        },
        "dataset": {
            ...
            "quality": {
                ...
                "expected_sequences": None if quality.expected_sequences is None else list(quality.expected_sequences),
        ...
        "execution_evidence": None if report.execution_evidence is None else {
            "execution_trace_fingerprint": report.execution_evidence.execution_trace_fingerprint,
            "cost_trace_fingerprint": report.execution_evidence.cost_trace_fingerprint,
            "portfolio_state_fingerprint": report.execution_evidence.portfolio_state_fingerprint,
            "valuation_ccy": report.execution_evidence.valuation_ccy,
            "fees_by_asset": [
                {"asset": asset, "amount": _decimal_text(amount)}
                for asset, amount in report.execution_evidence.fees_by_asset
            ],
            "funding": _decimal_text(report.execution_evidence.funding),
            "applied_fill_ids": list(report.execution_evidence.applied_fill_ids),
        },
...
def deserialize_backtest_report(value: object) -> BacktestReportSnapshot:
    ...
    try:
        ...
        run = BacktestRunFacts(
            ...
            None if run_value.get("cost_policy_fingerprint") is None else _text(run_value["cost_policy_fingerprint"], "run.cost_policy_fingerprint"),
        )
        ...
        quality = DataQualityAssessment(
            ...
            None if quality_value.get("expected_sequences") is None else tuple(quality_value["expected_sequences"]),
            quality_value.get("disconnected", False),
        )
        bars = []
        ...
        dataset = BacktestDatasetSnapshot(
            ...
            None if dataset_value.get("timeframe") is None else _text(dataset_value["timeframe"], "dataset.timeframe").lower(),
        )
        ...
        evidence_value = root.get("execution_evidence")
        evidence = None
        if evidence_value is not None:
            evidence_map = _mapping(evidence_value, "execution_evidence")
            fee_rows = evidence_map.get("fees_by_asset", [])
            if not isinstance(fee_rows, list):
                raise BacktestReportCodecError("execution_evidence.fees_by_asset must be a list")
            fees = []
            for index, row in enumerate(fee_rows):
                item = _mapping(row, f"execution_evidence.fees_by_asset[{index}]")
                fees.append((
                    _text(item["asset"], f"execution_evidence.fees_by_asset[{index}].asset").upper(),
                    _decimal(item["amount"], f"execution_evidence.fees_by_asset[{index}].amount"),
                ))
            fill_ids = evidence_map.get("applied_fill_ids", [])
            if not isinstance(fill_ids, list):
                raise BacktestReportCodecError("execution_evidence.applied_fill_ids must be a list")
            evidence = BacktestExecutionEvidence(
                _text(evidence_map["execution_trace_fingerprint"], "execution_evidence.execution_trace_fingerprint"),
                None if evidence_map.get("cost_trace_fingerprint") is None else _text(evidence_map["cost_trace_fingerprint"], "execution_evidence.cost_trace_fingerprint"),
                None if evidence_map.get("portfolio_state_fingerprint") is None else _text(evidence_map["portfolio_state_fingerprint"], "execution_evidence.portfolio_state_fingerprint"),
                _text(evidence_map["valuation_ccy"], "execution_evidence.valuation_ccy").upper(),
                tuple(fees),
                _decimal(evidence_map["funding"], "execution_evidence.funding"),
                tuple(_text(item, "execution_evidence.applied_fill_ids") for item in fill_ids),
            )
        report = BacktestReportSnapshot(
            ...
            metrics,
            windows,
            _utc(root["report_created_at"], "report_created_at"),
            evidence,
        )
