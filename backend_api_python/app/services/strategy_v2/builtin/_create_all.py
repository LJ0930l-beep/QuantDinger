"""Batch create 15 optimized strategies."""
import os
d = os.path.dirname(os.path.abspath(__file__))

STRATEGIES = {
    # ── 稳妥系列 (conservative, lower risk) ──
    "s01_bband_rsi.py": {
...
def initialize(context):
    ...
    context.set_benchmark(str(context.params.get("symbol", "Crypto:BTC/USDT@spot")))
...
'''
    ...
    # ── 激进系列 (aggressive, higher frequency) ──
    "a01_supertrend.py": {
...
def handle_data(context, data):
    ...
    tp = float(context.params.get("target_pct", 0.9))
    bars = get_history(lb + 10, str(context.params.get("frequency", "15m")), ["high", "low", "close", "open"], symbol)
