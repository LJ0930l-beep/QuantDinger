        ...
        *,
        ...
        try:
            ...
            fill_facts = normalize_gate_fills(
                ...
                # This service uses Gate's `/futures/usdt/*` endpoints, whose
                # fee denomination is the USDT settlement asset regardless of
                # the display valuation currency.
                default_fee_asset=("USDT" if auth.market_type.value == "perpetual" else None),
            ) if fills not in (None, (), []) else ()
            account_book_facts = normalize_gate_account_book(
                account_book, market_type=auth.market_type, account_scope=auth.account_scope,
                observed_at=observed_at, source_event_prefix="account-book",
            ) if account_book not in (None, (), []) else ()
            ...
            return build_gate_read_snapshot(
                ...
                orders=order_facts, fills=fill_facts, account_book=account_book_facts,
