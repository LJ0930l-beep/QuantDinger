...
from app.domain.backtest_report_contracts import BacktestExecutionEvidence, BacktestReportSnapshot, BacktestReportError, build_backtest_report
    ...
    def build(
        ...
        *,
        ...
        try:
            ...
            execution_evidence = BacktestExecutionEvidence(
                result.trace.trace_fingerprint,
                None if result.cost_trace is None else result.cost_trace.cost_trace_fingerprint,
                None if portfolio is None else portfolio.state_fingerprint,
                result.run.valuation_ccy,
                () if portfolio is None else portfolio.fees_by_asset,
                Decimal("0") if portfolio is None else portfolio.funding,
                () if portfolio is None else tuple(item.fill_id for item in portfolio.applied_fills),
            )
